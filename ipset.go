...
import (
	...
	utilexec "github.com/recall704/utils/exec"
...
func (set *IPSet) Validate() bool {
	...
	if set.HashSize <= 0 {
		log.Errorf("Invalid hashsize value %d, should be >0", set.HashSize)

		return false
	}
	// check max elem value of ipset
	if set.MaxElem <= 0 {
		log.Errorf("Invalid maxelem value %d, should be >0", set.MaxElem)
		return false
	}
...
func (e *Entry) Validate(set *IPSet) bool {
	if e.Port < 0 {
		log.Errorf("Entry %v port number %d should be >=0 for ipset %v", e, e.Port, set)
		return false
	}
	switch e.SetType {
	case HashIPPort:
	...
	case HashIPPortIP:
		...
		if net.ParseIP(e.IP2) == nil {
			log.Errorf("Error parsing entry %v second ip address %v for ipset %v", e, e.IP2, set)
			return false
		}
	case HashIPPortNet:
		//check if IP and Protocol of Entry is valid.
		...
		if _, ipNet, err := net.ParseCIDR(e.Net); ipNet == nil {
			log.Errorf("Error parsing entry %v ip net %v for ipset %v, error: %v", e, e.Net, set, err)
			return false
		}
	case BitmapPort:
		// check if port number satisfies its ipset's requirement of port range
		if set == nil {
			log.Errorf("Unable to reference ip set where the entry %v exists", e)
			return false
		}
		begin, end, err := parsePortRange(set.PortRange)
		if err != nil {
			log.Errorf("Failed to parse set %v port range %s for ipset %v, error: %v", set, set.PortRange, set, err)
			return false
		}
		if e.Port < begin || e.Port > end {
			log.Errorf("Entry %v port number %d is not in the port range %s of its ipset %v", e, e.Port, set.PortRange, set)
			return false
		}
	}
...
func (e *Entry) checkIPandProtocol(set *IPSet) bool {
	...
	if net.ParseIP(e.IP) == nil {
		log.Errorf("Error parsing entry %v ip address %v for ipset %v", e, e.IP, set)
		return false
	}
...
func validatePortRange(portRange string) bool {
	...
	if len(strs) != 2 {
		log.Errorf("port range should be in the format of `a-b`")
		return false
	}
	for i := range strs {
		num, err := strconv.Atoi(strs[i])
		if err != nil {
			log.Errorf("Failed to parse %s, error: %v", strs[i], err)
			return false
		}
		if num < 0 {
			log.Errorf("port number %d should be >=0", num)
			return false
		}
	}
	return true
...
func validateIPSetType(set Type) bool {
	...
	log.Errorf("Currently supported ipset types are: %v, %s is not supported", ValidIPSetTypes, set)
	return false
}
...
func validateHashFamily(family string) bool {
	...
	log.Errorf("Currently supported ip set hash families are: [%s, %s], %s is not supported", ProtocolFamilyIPV4, ProtocolFamilyIPV6, family)
	return false
}
...
func validateProtocol(protocol string) bool {
	...
	log.Errorf("Invalid entry's protocol: %s, supported protocols are [%s, %s, %s]", protocol, ProtocolTCP, ProtocolUDP, ProtocolSCTP)
	return false
}
